...
Then(/^I navigate to Welcome to Mail plus Editions page/, async () => {
  ...
  await expect(await welcomeToMailPlusEditionsPage.mailPlusEditionsHeaderText).toBeDisplayed()
})
...
Then(/^I should see the "([^"]*)" edition has been downloaded$/, async (date) => {
  ...
  await expect(await newspaperPdfPage.bookIcon).toBeEnabled()
})
...
Then(/^I should be returned back to the ALB page$/,  async () => {
	await expect(await newspaperPdfPage.cameraIconOnImage).toBeDisplayed()
})
